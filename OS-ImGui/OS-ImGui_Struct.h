...
public:
	...
	Vec4 operator+(Vec4 Vec3_)
	{
		return { x + Vec3_.x,y + Vec3_.y,z + Vec3_.z ,w+ Vec3_.w };
	}
	Vec4 operator-(Vec4 Vec3_)
	{
		return { x - Vec3_.x,y - Vec3_.y,z - Vec3_.z,w - Vec3_.w };
	...
	float DistanceTo(const Vec4& Pos)
	{
		return sqrtf(powf(Pos.x - x, 2) + powf(Pos.y - y, 2) + powf(Pos.z - z, 2) + powf(Pos.w - w, 2));
	}
};
