...
namespace OSImGui
{
    ...
    void  OSImGui_External::MyWindow(std::function<void()> DrawCallBack)
    {
        std::string DestWindowName = "Program Manager";
        std::string DestWindowClassName = "Progman";
        ...
        DestWindow.hWnd = FindWindowA(DestWindowClassName.c_str(), DestWindowClassName.c_str());
        DestWindow.Name = DestWindowName;
        DestWindow.ClassName = DestWindowClassName;

        Type = MYTYPE;
        CallBackFn = DrawCallBack;
    ...
    void OSImGui_External::MainLoop()
    {
        while (!EndFlag)
        {
            ...
            g_Device.g_pSwapChain->Present(1, 0); // （1，0）开启垂直同步
        }
        CleanImGui();
    }
    ...
    bool OSImGui_External::UpdateWindowData()
    {
        ...
        if (this->Type == ATTACH) {
            ...
            Window.Pos = DestWindow.Pos = Vec2(0.f, 0.f);
            Window.Size = DestWindow.Size = Vec2(static_cast<float>(GetSystemMetrics(SM_CXSCREEN)), static_cast<float>(GetSystemMetrics(SM_CYSCREEN)));
        }
